...
@struct.dataclass
class Report:
    # in the order of events happening:
    all_units_removed_due_to_reset: jnp.zeros((), dtype=bool)
    #
    units_removed_due_to_negative_energy: jnp.zeros((2, 16), dtype=bool)
    units_have_moved: jnp.zeros((2, 16), dtype=bool)
    units_have_sapped: jnp.zeros((2, 16), dtype=bool)
    #
    sapping_energy_benefit: jnp.zeros((2, ), dtype=jnp.int16)
    n_units_destroyed_by_collision: jnp.zeros((2,), dtype=jnp.int16)
    #
    energy_variation_from_sap: jnp.zeros((2, 16), dtype=jnp.int16)
    energy_variation_from_void: jnp.zeros((2, 16), dtype=jnp.int16)
    energy_variation_from_field: jnp.zeros((2, 16), dtype=jnp.int16)
    #
    units_have_spawned: jnp.zeros((2, 16), dtype=bool)
...
class LuxAIS3Env(environment.Environment):
    ...
    def step_env(
        ...
        all_units_removed_due_to_reset = state.match_steps == 0  # AL
        ...
        units_removed_due_to_negative_energy = (state.units.energy[..., 0] < 0) & state.units_mask  # AL
        state = state.replace(
        ...
        new_units, units_have_moved = jax.vmap(  # AL
        ...
        def sap_unit(
            ...
            for t in range(self.fixed_env_params.num_teams):
                ...
                previous_energy_sum = jnp.sum(units_mask[..., None] * all_units.energy, axis=(-1, -2), dtype=jnp.int16)  # (2, )
                ...
                sapping_units = sapping_units.at[t, :].set(team_unit_sapped)  # AL
                ...
                # AL
                new_energy_sum = jnp.sum(units_mask[..., None] * all_units.energy, axis=(-1, -2), dtype=jnp.int16)  # (2, )
                energy_sum_variation = new_energy_sum - previous_energy_sum
                sapping_energy_benefit = sapping_energy_benefit.at[t].set(energy_sum_variation[t] - energy_sum_variation[1-t])

            return all_units, sapping_units, sapping_energy_benefit

        sapping_units = jnp.zeros((2, 16), dtype=bool)  # AL
        sapping_energy_benefit = jnp.zeros((2,), dtype=jnp.int16)  # AL

        previous_energy = deepcopy(state.units.energy)

        new_units, sapping_units, sapping_energy_benefit = sap_unit(
        ...
        energy_variation_from_sap = state.units_mask[:, :, None] * (state.units.energy - previous_energy)  # AL
        ...
        previous_n_units = jnp.sum(state.units_mask, axis=-1)  # (2,)
        for t in range(self.fixed_env_params.num_teams):
            other_team_ids = jnp.array(
                [t2 for t2 in range(self.fixed_env_params.num_teams) if t2 != t]
            )
        ...
        new_n_units = jnp.sum(state.units_mask, axis=-1)  # (2,)
        n_units_destroyed_by_collision = previous_n_units - new_n_units  # (2,)

        # apply energy void fields
        previous_energy = deepcopy(state.units.energy)  # AL
        for t in range(self.fixed_env_params.num_teams):
            other_team_ids = jnp.array(
                [t2 for t2 in range(self.fixed_env_params.num_teams) if t2 != t]
            )
        ...
        energy_variation_from_void = state.units_mask[:, :, None] * (state.units.energy - previous_energy)  # AL
        ...
        energy_variation_from_field = state.units_mask[:, :, None] * (state.units.energy - previous_energy)  # AL
        ...
        previous_units_masks = deepcopy(state.units_mask)  # AL
        state = jax.lax.cond(
            spawn_units_in, lambda: spawn_team_units(state), lambda: state
        )
        units_have_spawned = (state.units_mask.astype(jnp.int16) - previous_units_masks.astype(jnp.int16)).astype(bool)  # AL
        ...
        report = Report(
            all_units_removed_due_to_reset=all_units_removed_due_to_reset,
            units_removed_due_to_negative_energy=units_removed_due_to_negative_energy,
            units_have_moved=units_have_moved.squeeze(-1),
            units_have_sapped=sapping_units,
            sapping_energy_benefit=sapping_energy_benefit,
            n_units_destroyed_by_collision=n_units_destroyed_by_collision,
            energy_variation_from_sap=energy_variation_from_sap.squeeze(-1).astype(jnp.int16),
            energy_variation_from_void=energy_variation_from_void.squeeze(-1).astype(jnp.int16),
            energy_variation_from_field=energy_variation_from_field.squeeze(-1).astype(jnp.int16),
            units_have_spawned=units_have_spawned,
        )
        return (
            ...
            terminated,
            truncated,
            {"discount": self.discount(state, params), "report": report},
        )
