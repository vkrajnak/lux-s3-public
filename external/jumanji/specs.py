...
from external.jumanji.testing.pytrees import is_equal_pytree
from external.jumanji.types import get_valid_dtype

T = TypeVar("T")


class Spec(abc.ABC, Generic[T]):
    """Adapted from `dm_env_spec.Array`. This is an augmentation of the `Array` spec to allow for
...
class Array(Spec[chex.Array]):
    """Describes a jax array spec. This is adapted from `dm_env_specs.Array` for Jax environments.
...
class BoundedArray(Array):
    ...
    adapted from `dm_env_specs.BoundedArray` to suit Jax environments.
...
class DiscreteArray(BoundedArray):
    ...
    `dm_env_specs`.BoundedArray to suit Jax environments.
...
def jumanji_specs_to_dm_env_specs(
    ...
    dm_env_specs.DiscreteArray,
    dm_env_specs.BoundedArray,
    dm_env_specs.Array,
    Dict[str, dm_env_specs.Array],
]:
    ...
    Returns:
        `dm_env_specs.Array` or pytree of `dm_env_specs.Array` corresponding to the equivalent
